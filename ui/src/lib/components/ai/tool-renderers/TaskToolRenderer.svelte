...
<div class="flex items-center justify-between gap-4 px-4 pt-4">
	<CollapsibleTrigger class="flex min-w-0 flex-1 items-center gap-2 text-left">
		...
		<span class="truncate font-medium text-sm">
			{validInput?.description || validInput?.subagent_type || (isStreaming ? "Loading task details..." : "Sub-agent task")}
		</span>
...
<ToolContent>
	{#if !toolPart.input || typeof toolPart.input !== "object"}
		<div class="p-4 pt-3 text-muted-foreground text-sm">{isStreaming ? "Loading task details..." : "Task details are unavailable."}</div>
	{:else if !inputValidation.success}
		<div class="space-y-3 p-4 pt-3">
			<p class="text-muted-foreground text-sm">{isStreaming ? "Loading task details..." : "Could not parse task details."}</p>
			{#if rawOutputText}
					...
					<div class="space-y-2 rounded-md border bg-muted/20 p-3 text-xs">
						...
						{#if validOutput?.output_file}
							<div class="flex items-center gap-2">
								...
								<code class="font-mono text-foreground">{shortenPath(validOutput.output_file)}</code>
							</div>
						{/if}
					</div>
				</div>
