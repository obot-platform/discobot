<script lang="ts">
	...
	const previewContent = $derived.by(() => validInput?.content?.slice(0, 400) || "");
	const writeError = $derived.by(() => toolPart.errorText || validOutput?.error);
...
<div class="flex items-center justify-between gap-4 px-4 pt-4">
	<CollapsibleTrigger class="flex min-w-0 flex-1 items-center gap-2 text-left">
		...
		<span class="truncate font-medium text-sm">
			{validInput?.file_path ? shortenPath(validInput.file_path) : isStreaming ? "Loading write details..." : "Write file"}
		</span>
...
<ToolContent>
	{#if !toolPart.input || typeof toolPart.input !== "object"}
		<div class="p-4 pt-3 text-muted-foreground text-sm">
			{isStreaming ? "Loading write details..." : "Write details are unavailable."}
		</div>
	{:else if !inputValidation.success || !validInput?.file_path}
		<div class="space-y-3 p-4 pt-3">
			<p class="text-muted-foreground text-sm">
				{isStreaming ? "Loading write details..." : "Could not parse write details."}
			</p>
			{#if rawOutputText}
			...
			<div class="space-y-3 rounded-md border bg-muted/20 p-3">
				...
				{#if previewContent}
					<div class="rounded-md border bg-background/80">
						<pre class="max-h-56 overflow-auto whitespace-pre-wrap break-words p-3 font-mono text-xs"><code>{previewContent}</code></pre>
					</div>
				{/if}
			</div>
			...
			<div class="flex flex-wrap items-center gap-3 text-xs">
				{#if validOutput?.success !== undefined}
					<span class={validOutput.success ? "text-green-700" : "text-yellow-700"}>
						{validOutput.success ? "Write completed" : "Write returned warnings"}
					</span>
				{/if}
