...
<ToolContent>
	{#if !toolPart.input || typeof toolPart.input !== "object"}
		<div class="p-4 pt-3 text-muted-foreground text-sm">{isStreaming ? "Loading file search..." : "File search details are unavailable."}</div>
	{:else if !inputValidation.success || !validInput?.pattern}
		<div class="space-y-3 p-4 pt-3">
			<p class="text-muted-foreground text-sm">
				{isStreaming ? "Loading file search..." : "Could not parse file search details."}
			</p>
			{#if rawOutputText}
	...
	{:else}
		<div class="space-y-4 p-4 pt-3">
			...
			{#if validOutput?.files?.length}
				<div class="space-y-2">
					<p class="text-muted-foreground text-xs">{validOutput.files.length} files matched</p>
