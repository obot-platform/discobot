<script lang="ts">
	...
	type Props = SchemaParameter & { class?: string };
	let { name, type, required, description, location, class: className, ...restProps }: Props = $props();
</script>

<div class={cn("px-4 py-3 pl-10", className)} {...restProps}>
