...
<div class="space-y-4">
	...
	<div class="grid overflow-hidden rounded-2xl border border-border bg-background/70 xl:grid-cols-[minmax(0,1fr)_13rem]">
		<div class="min-w-0 border-b border-border xl:border-r xl:border-b-0">
			<div class="flex flex-wrap items-center gap-1 border-b border-border px-2 py-2">
				{#each files.slice(0, 4) as file}
					...
					class={`rounded-lg px-3 py-1.5 text-sm transition ${selectedFile === file ? "bg-card text-foreground shadow-sm" : "text-muted-foreground hover:bg-accent"}`}
					>
						{fileLabel(file)}
					</button>
				{/each}
			...
			<div class="space-y-3 p-4">
				...
				<pre class="overflow-x-auto rounded-xl border border-border bg-card p-4 font-mono text-sm leading-6 text-muted-foreground"><code>{fileContents[selectedFile]}</code></pre>
			</div>
		</div>
			...
			<div class="mt-3 space-y-2">
				{#each files as file, index}
					<button
						type="button"
						onclick={() => onSelectFile(file)}
					class={`block w-full rounded-xl border px-3 py-3 text-left ${selectedFile === file ? "border-primary bg-primary/10" : "border-border bg-card/70"}`}
					>
						<div class="flex items-center justify-between gap-3">
							<div class="min-w-0">
