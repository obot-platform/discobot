...
class AppContextState {
    ...
    defaultModel = $state("");
    settingsDialogTab = $state<SettingsDialogTab>("appearance");
    credentialFlowIntent = $state<"github-git" | null>(null);
    ...
    workspacesStatus = $state<AsyncStatus>("idle");
    ...
    recentSessions = $derived.by(() =>
        this.sessions.filter((session) => session.isRecent),
    );
    ...
    private workspacesRequest: Promise<void> | null = null;
    ...
    constructor(bootstrap: AppContextBootstrap) {
        ...
        this.selectedSessionId =
            ...
            bootstrap.sessions.some(
                (session) => session.id === bootstrap.selectedSessionId,
            )
                ? bootstrap.selectedSessionId
                : null;
    ...
    private ensureColorSchemeForMode = () => {
        if (
            !this.availableThemes.some((theme) => theme.id === this.colorScheme)
        ) {
    ...
    private resolveWorkspaceId = async (): Promise<string | null> => {
        ...
        const workspace =
            workspaces.find((candidate) => candidate.status === "ready") ||
            workspaces[0];
        if (!workspace) {
            return null;
        }
    ...
    private resolveAgentId = async (): Promise<string | null> => {
        ...
        const agent =
            agents.find((candidate) => candidate.isDefault) || agents[0];
        if (!agent) {
            return null;
        }
    ...
    refreshWorkspaces = async () => {
        if (this.workspacesRequest) {
            await this.workspacesRequest;
            return;
        }

        this.workspacesRequest = (async () => {
            this.workspacesStatus = "loading";
            try {
                const { workspaces } = await api.getWorkspaces();
                this.workspaces = workspaces;
                this.workspacesStatus = "ready";
                this.errorMessage = undefined;
            } catch (error) {
                this.workspacesStatus = "error";
                this.errorMessage = this.formatErrorMessage(
                    error,
        ...
        try {
            ...
            this.errorMessage =
                error instanceof Error
                    ? error.message
                    : "Failed to initialize app context";
        }
    };
    ...
    renameSession = async (sessionId: string, nextName: string): Promise<boolean> => {
        ...
        try {
            await api.updateSession(sessionId, { displayName: trimmedName });
    ...
    createSessionForWorkspace = async (
        workspaceId?: string,
    ): Promise<string | null> => {
        try {
    ...
    openSettingsDialog = () => {
        ...
        this.settingsDialogTab = "appearance";
        this.settingsDialogOpen = true;
    };

    openSettingsDialogAt = (tab: SettingsDialogTab) => {
        this.credentialsDialogOpen = false;
        this.supportInfoDialogOpen = false;
        this.settingsDialogTab = tab;
        this.settingsDialogOpen = true;
    };
    ...
    closeSettingsDialog = () => {
        ...
        this.settingsDialogTab = "appearance";
        this.credentialFlowIntent = null;
    };

    openCredentialsDialog = () => {
        this.openSettingsDialogAt("credentials");
    };

    openGitHubCredentialFlow = () => {
        this.credentialFlowIntent = "github-git";
        this.openSettingsDialogAt("credentials");
    };
