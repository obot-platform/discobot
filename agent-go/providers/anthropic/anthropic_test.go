...
func TestCountTokens(t *testing.T) {
	t.Run("calls count_tokens endpoint", func(t *testing.T) {
		server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
			if r.URL.Path != "/messages/count_tokens" {
				t.Errorf("expected /messages/count_tokens, got %s", r.URL.Path)
			}
			if r.Method != "POST" {
				t.Errorf("expected POST, got %s", r.Method)
			}
			...
			// stream should not be present in count_tokens request.
			if _, ok := body["stream"]; ok {
				t.Error("unexpected stream field in count_tokens request")
			}
	...
	t.Run("handles API error", func(t *testing.T) {
		server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, _ *http.Request) {
			w.WriteHeader(500)
			fmt.Fprint(w, `{"type":"error","error":{"type":"api_error","message":"internal"}}`)
		}))
		defer server.Close()
