...
- The redesign diff review panel now renders real per-file session diffs with `@pierre/diffs`, including split/unified mode, syntax highlighting, clickable collapsed unchanged regions, patch-hash approval persistence, and deleted-file snapshot reconstruction with a base-file fallback when needed.
...
- The Svelte redesign app context now owns a project-events SSE subscription that reloads only the affected session/workspace objects for update events, while still doing full refreshes on reconnect, coalescing overlapping app/session/thread reload requests behind per-scope locks to avoid refresh stampedes, and tracking startup tasks for a global startup-status banner during backend initialization work.
