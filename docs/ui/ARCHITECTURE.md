...
- The redesign diff review panel now renders real per-file session diffs with `@pierre/diffs`, including split/unified mode, worker-pool syntax highlighting, stable diff input caching to avoid unnecessary rerenders, lazy per-file diff loading, virtualized rendering for large inline diffs, patch-hash approval persistence, and deleted-file snapshot reconstruction with a base-file fallback when needed.
